...
OpenDesk TW 2.2.2 不修改 `/Applications/ONLYOFFICE.app` 或 Windows 的已簽署程式檔，而是在使用者自己的設定與外掛資料夾完成修復，方便更新、回復與熱修。
...
OpenDesk TW 自己的畫面與新增工具均為繁體中文，並會清除已知簡體範本快取。2.2.2 起會讀取 ONLYOFFICE 公開的英文、簡體與繁體語系對照，針對上游仍沿用英文的欄位產生台灣繁中覆寫，並在對話框、提示卡與選單出現時即時套用。覆寫層含 11,000 組以上對照，使用 OpenCC 台灣詞庫及 OpenDesk 術語修正；不修改已簽章的 ONLYOFFICE.app，也不連網翻譯文件內容。

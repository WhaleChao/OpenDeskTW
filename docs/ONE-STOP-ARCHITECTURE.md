...
- ONLYOFFICE 內的「MAGI」頁籤經由隨機權杖保護的本機橋接呼叫相容層；原有通用 AI 另列為「其他 AI 模型」，兩者不混稱。
...
- 編輯器橋接只監聽 `127.0.0.1`，限制來源、本文大小與請求方法；外掛只取得每次 App 啟動重建的短期權杖，不接觸 MAGI API 金鑰。

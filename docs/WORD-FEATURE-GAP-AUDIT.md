...
一般 DOCX 編輯、頁面配置、表格／圖片、目錄、註腳／尾註、標號、交互參照、追蹤修訂、比較合併、密碼與限制編輯，已可由工作台與 ONLYOFFICE 完成。2.6.2 另修正三個會直接影響台灣文件工作的缺口：可寫回 OOXML 的原生分散對齊、`PMingLiU`／`MingLiU` 精確字型選取、依巢狀前文即時決定 `」`／`』`；工作台也會拒絕把舊版繁中外掛誤報為已就緒。

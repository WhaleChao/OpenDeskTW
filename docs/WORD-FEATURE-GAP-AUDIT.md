...
一般 DOCX 編輯、頁面配置、表格／圖片、目錄、註腳／尾註、標號、交互參照、追蹤修訂、比較合併、密碼與限制編輯，已可由工作台與 ONLYOFFICE 完成。2.6.2 新增 `PMingLiU`／`MingLiU` 精確名稱及巢狀下引號；2.7.3 修正為 ONLYOFFICE 9.4 真正會重排畫面的 Word 式文字等距分布命令，並從本機已授權 Microsoft Office 註冊新細明體／細明體、補上 Word 格式快捷鍵；2.7.4 再於每次存檔成功後把已標記段落寫成 Word 標準 `w:jc="distribute"`，不再只依賴安裝外掛後的重開還原；2.7.5 補齊 Windows 暫存檔驗證後的檔案鎖釋放，確保兩平台都能安全置換 DOCX。
...
| 字型與段落 | 完整／可驗證 | 常用字元格式、樣式、行距、定位點、格式複製；原生編輯命令套用 `distribute=4`，2.7.5 在存檔成功後把 `w:jc="distribute"` 寫回 DOCX，並通過 Windows／macOS 寫回測試；macOS 從本機已授權 Microsoft Office 註冊新細明體／細明體 | 沒有已授權 Microsoft Office 字型來源的電腦會以替代字型顯示，但 DOCX 仍保留要求的家族名稱 |

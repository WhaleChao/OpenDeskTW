...
一般 DOCX 編輯、頁面配置、表格／圖片、目錄、註腳／尾註、標號、交互參照、追蹤修訂、比較合併、密碼與限制編輯，已可由工作台與 ONLYOFFICE 完成。2.6.2 新增 `PMingLiU`／`MingLiU` 精確名稱及巢狀下引號；2.7.3 加入文字等距分布及 Word 格式快捷鍵；2.7.4 於每次存檔成功後把已標記段落寫成 Word 標準 `w:jc="distribute"`；2.7.6 改為依每行實際可用寬度動態計算、寬度改變即重算，存檔時移除畫面暫時字距，並把格式複製／套用改接 ONLYOFFICE 原生格式剪貼簿，實際驗證 macOS `Command+Shift+C/V` 與 Windows／Linux `Ctrl+Shift+C/V`，同時補齊 Windows 暫存檔驗證讀取器釋放與可寫入同步句柄，確保兩平台安全置換 DOCX。2.7.7 修正未命名空白段落必須以數字配置 `ParaId`，並對 twips 與字型寬度量化保留動態安全邊界；若套用後行數增加，會自動縮回字距重排，避免最後一字凸到下一行。
...
| 字型與段落 | 完整／可驗證 | 常用字元格式、樣式、行距、定位點、格式複製；依每行頁面／儲存格／縮排寬度動態重排，存檔清除暫時字距並寫回 `w:jc="distribute"`；Ctrl／Command+Shift+C/V 以真實粗體來源與目標文字 LIVE 驗證；macOS 從本機已授權 Microsoft Office 註冊新細明體／細明體 | 沒有已授權 Microsoft Office 字型來源的電腦會以替代字型顯示，但 DOCX 仍保留要求的家族名稱 |

...
一般 DOCX 編輯、頁面配置、表格／圖片、目錄、註腳／尾註、標號、交互參照、追蹤修訂、比較合併、密碼與限制編輯，已可由工作台與 ONLYOFFICE 完成。2.6.2 新增 `PMingLiU`／`MingLiU` 精確名稱及巢狀下引號；2.7.3 修正為 ONLYOFFICE 9.4 真正會重排畫面的 Word 式文字等距分布命令，並以文件內段落標記解決 9.4 存檔正規化後的重開還原；同版也從本機已授權 Microsoft Office 註冊新細明體／細明體，並補上 Ctrl+Shift+C／V 與 macOS Word 格式快捷鍵。
...
| 字型與段落 | 完整／可驗證 | 常用字元格式、樣式、行距、定位點、格式複製；2.7.3 由原生編輯命令套用 `distribute=4`、以文件標記重開還原，並在 macOS 從本機已授權 Microsoft Office 註冊新細明體／細明體 | ONLYOFFICE 9.4 寫入 DOCX 時仍會把 `w:jc` 正規化；文字等距分布需由安裝本外掛的 OpenDesk／ONLYOFFICE 重開才會自動還原。沒有已授權 Microsoft Office 字型來源的電腦則以替代字型顯示，但 DOCX 會保留要求的家族名稱 |

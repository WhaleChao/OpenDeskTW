...
目前開發版為 2.7.0，主要程式位於 `cross-platform/`，以 Tauri 2、Rust 與原生 WebView 建置。

## 主要能力

- 文字文件：DOCX／DOCM 健康報告、標題地圖、字型與列印檢查、頁碼與頁首頁尾檢查、註解與修訂檢查。
- 台灣繁中寫作：ONLYOFFICE 固定 `zh-TW`、數字字級、可寫回 DOCX `distribute` 的原生分散對齊、`PMingLiU`／`MingLiU` 精確選取、即時巢狀下引號、台灣標點與可搜尋快捷鍵總覽。

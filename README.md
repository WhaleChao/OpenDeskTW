...
目前開發版為 2.8.3，主要程式位於 `cross-platform/`，以 Tauri 2、Rust 與原生 WebView 建置。新增文字文件、試算表或簡報會立即開啟未命名文件，第一次按儲存時才選擇檔名與位置；Word 編輯器另有每 15 秒富文字草稿、可編輯大綱、完整條件合併、動態引文、台灣文件校閱與結構化範本。2.8.1 修正表格內文字等距分布；2.8.2 取消打字與普通點擊後的全文件重排；2.8.3 解除 macOS 系統顏色面板對 `⇧⌘C` 的攔截，讓 Word 格式複製／套用熱鍵可真正送進編輯器。

## 主要能力

- 文字文件：DOCX／DOCM 健康報告、標題地圖、字型與列印檢查、頁碼與頁首頁尾檢查、註解與修訂檢查。
- 台灣繁中寫作：ONLYOFFICE 固定 `zh-TW`、數字字級、會真正重排並在 DOCX 寫入 Word 標準 `w:jc="distribute"` 的文字等距分布、從本機已授權 Microsoft Office 註冊 `PMingLiU`／`MingLiU`、即時巢狀下引號、台灣標點與可搜尋快捷鍵總覽。
- 中文標題安全重編：只辨識段落開頭的「壹、」、「一、」、「（一）」與 `1.`，內文中出現不會誤判；先備份再建立套用標題樣式的新副本。
- Word 進階工具：45 秒磁碟快照、15 秒記憶體富文字草稿、完整條件合併列印、信封／標籤、擴充無障礙檢查、本機動態引文及 APA／MLA／Chicago／台灣書目。
- Word 重度工作：可調整標題層級與章節順序的大綱、草稿／Web 與並排同步、註解與 `@` 指派、MAGI 翻譯／改寫、封面、AutoText、四種結構化範本及可繼續編輯的開放式圖解。
...
- MAGI：主視窗與 ONLYOFFICE 內都有文件分析入口；以權杖保護的 `127.0.0.1` 橋接相容目前唯一運作的 MAGI V2 或 V3，會自動重讀權杖、先做健康檢查並清楚提示工作台未開啟，結果留在 App，不自行啟停 Agent、不改開網頁。
- AI 分流：ONLYOFFICE 原有 AI 明確顯示為「其他 AI 模型」，使用獨立台灣繁中相容副本；它不是 MAGI，不會混用名稱或設定。
...
與 Microsoft Word 桌面版逐類比對的功能差距、排除項目與建議開發順序，見 `docs/WORD-FEATURE-GAP-AUDIT.md`。

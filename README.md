...
目前開發版為 2.6.1，主要程式位於 `cross-platform/`，以 Tauri 2、Rust 與原生 WebView 建置。

## 主要能力

- 文字文件：DOCX／DOCM 健康報告、標題地圖、字型與列印檢查、頁碼與頁首頁尾檢查、註解與修訂檢查。
- 台灣繁中寫作：ONLYOFFICE 固定 `zh-TW`、數字字級、逐行填滿行寬的分散對齊、`PMingLiU`／`MingLiU` 精確選取、即時巢狀下引號、台灣標點與可搜尋快捷鍵總覽。
...
- MAGI：主視窗與 ONLYOFFICE 內都有文件分析入口；以權杖保護的 `127.0.0.1` 橋接相容目前唯一運作的 MAGI V2 或 V3，結果留在 App，不自行啟停 Agent、不改開網頁。

...
目前開發版為 2.7.7，主要程式位於 `cross-platform/`，以 Tauri 2、Rust 與原生 WebView 建置。新增文字文件、試算表或簡報會立即開啟未命名文件，第一次按儲存時才選擇檔名與位置；macOS 引擎版本偵測也不會執行 LibreOffice。
...
- Word 進階工具：45 秒工作階段快照與另存復原、CSV／TSV 合併列印、信封／標籤範本、逐項定位的無障礙檢查與安全修復、本機引文來源及 APA／MLA／Chicago／台灣書目。
- Word 重度工作：大綱／草稿／Web 與並排同步檢視、註解及 `@` 指派清單、MAGI 翻譯／改寫／同義詞／相似度、封面、AutoText，以及可繼續編輯的流程／階層／矩陣圖解。

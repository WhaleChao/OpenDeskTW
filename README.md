...
目前開發版為 2.7.3，主要程式位於 `cross-platform/`，以 Tauri 2、Rust 與原生 WebView 建置。新增文字文件、試算表或簡報會立即開啟未命名文件，第一次按儲存時才選擇檔名與位置；macOS 引擎版本偵測也不會執行 LibreOffice。

## 主要能力

- 文字文件：DOCX／DOCM 健康報告、標題地圖、字型與列印檢查、頁碼與頁首頁尾檢查、註解與修訂檢查。
- 台灣繁中寫作：ONLYOFFICE 固定 `zh-TW`、數字字級、會真正重排且重開自動還原的 Word 式文字等距分布、從本機已授權 Microsoft Office 註冊 `PMingLiU`／`MingLiU`、即時巢狀下引號、台灣標點與可搜尋快捷鍵總覽。
